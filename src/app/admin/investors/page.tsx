...
import { BarChart3, Download, Mail, Presentation, UsersRound } from "lucide-react";
import { Container } from "@/components/ui/container";
import { PageHero } from "@/components/ui/page-hero";
import { requireAdminSession } from "@/lib/admin/session";
import { getInvestorDeckDashboardData } from "@/lib/investors/dashboard-data";
import { buildMetadata } from "@/lib/seo";

export const metadata: Metadata = buildMetadata({
  title: "Investor Presentation Analytics",
  description: "Protected Temacore investor presentation analytics.",
...
export default async function InvestorAnalyticsPage() {
  ...
  const dashboard = await getInvestorDeckDashboardData();
  const hasActivity = Object.values(dashboard.counts).some((count) => count > 0);
  ...
  return (
    <>
      <PageHero
        ...
        title="Investor presentation analytics."
        body="Review privacy-conscious, anonymous engagement events from the Temacore investor presentation."
      />
        ...
        <Container className="space-y-8">
          <div className="flex flex-col gap-4 rounded-lg border border-line bg-white p-6 shadow-[0_24px_70px_rgba(15,23,42,0.08)] md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <h2 className="mt-4 text-2xl font-black text-ink">Investor deck activity</h2>
          ...
          {dashboard.isConfigured && !hasActivity ? (
            <div className="rounded-lg border border-line bg-white p-6 text-sm font-semibold text-slate-600">
              No investor presentation activity has been recorded yet.
            </div>
          ) : null}

          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            <MetricCard icon={UsersRound} label="Deck opens" value={dashboard.counts.opens} showValue={dashboard.isConfigured} />
            <MetricCard icon={Presentation} label="Presentations started" value={dashboard.counts.starts} showValue={dashboard.isConfigured} />
            <MetricCard icon={BarChart3} label="Presentations completed" value={dashboard.counts.completions} showValue={dashboard.isConfigured} />
            <MetricCard icon={Download} label="PDF downloads" value={dashboard.counts.downloads} showValue={dashboard.isConfigured} />
          </div>

          <div className="grid gap-5 md:grid-cols-3">
            <InsightCard label="Contact clicks" value={dashboard.isConfigured ? String(dashboard.counts.contactClicks) : "Unavailable"} />
            <InsightCard
              label="Most frequently reached slide"
              value={dashboard.mostFrequentlyReachedSlide ? `Slide ${dashboard.mostFrequentlyReachedSlide}` : dashboard.isConfigured ? "No slide activity yet" : "Unavailable"}
            />
            <InsightCard
              label="Average furthest slide reached"
              value={dashboard.averageFurthestSlide ? `Slide ${dashboard.averageFurthestSlide}` : dashboard.isConfigured ? "No slide activity yet" : "Unavailable"}
            />
          </div>
                  ...
                  <div key={event.id} className="rounded-md bg-paper p-4">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <span className="text-xs font-bold uppercase tracking-[0.14em] text-blue002">
                        {event.slide_number ? `Slide ${event.slide_number}` : "Presentation"}
                      </span>
                    </div>
                    <p className="mt-2 text-xs font-semibold text-slate-600">
                      Session {event.session_id.slice(0, 8)} · {new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" }).format(new Date(event.created_at))}
                    </p>
                  </div>
                ))
              ) : (
